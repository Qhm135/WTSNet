...
class CBAMLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        avg_out = torch.mean(x, dim=1, keepdim=True)
        spatial_att = self.sigmoid(self.spatial(torch.cat([max_out, avg_out], dim=1)))
        x = x * spatial_att
        return x


class MySlic(nn.Module):
    def __init__(self, num_segments=100):
        super(MySlic, self).__init__()
...
#             seg = slic(img_np, n_segments=self.num_segments, compactness=10)
#             seg = torch.tensor(seg, dtype=torch.float32).unsqueeze(0).to(x.device)
#             seg = seg.unsqueeze(0)
#             edge = kornia.filters.sobel(seg)
#             edges.append(edge)
#
#         return torch.cat(edges, dim=0)  # [B, 1, H, W]
...
#         self.cbam = CBAMBlock(1)
#
#     def forward(self, x):
#         x_np = x.cpu().numpy().transpose(0, 2, 3, 1)
#         output = []
#         for i in range(x_np.shape[0]):
#             seg = slic(x_np[i], n_segments=self.n_segments, compactness=self.compactness)
#             output.append(seg)
#         sp_tensor = torch.tensor(np.stack(output, axis=0), dtype=torch.float32).to(x.device).unsqueeze(1)
#         sp_tensor = self.cbam(sp_tensor)
#         return sp_tensor
...
#                 x = F.pad(x, (0, x.shape[3] % 2, 0, x.shape[2] % 2), mode='reflect')
#
#             x01 = x[:, :, 0::2, :] / 2
#             x02 = x[:, :, 1::2, :] / 2
#             x1 = x01[:, :, :, 0::2]
...
#     def __init__(self, in_channels_per_subband=3):
#         super().__init__()
#         self.in_channels = in_channels_per_subband
#
#         self.conv_block = nn.Sequential(
#             nn.Conv2d(3 * self.in_channels, 4 * self.in_channels, kernel_size=3, padding=1),
#             nn.ReLU(),
#             nn.Conv2d(4 * self.in_channels, 3 * self.in_channels, kernel_size=3, padding=1),
#             nn.Sigmoid()
#         )
#
#     def forward(self, LH, HL, HH):
#         combined = torch.cat([LH, HL, HH], dim=1)
#         attention = self.conv_block(combined)
#         att_LH, att_HL, att_HH = torch.chunk(attention, 3, dim=1)
#         return LH * att_LH, HL * att_HL, HH * att_HH
#
#
...
#         super().__init__()
#         self.wave = wave
#         self.attention = FrequencyAdaptiveAttention(in_channels_per_subband=3)
#         self.subband_fusion = SubbandFusion(12)
#     def forward(self, x):
...
#                 LL, (LH, HL, HH) = pywt.dwt2(channel_np[b, 0], self.wave)
#
#                 subbands['LL'].append(torch.tensor(LL, device=x.device).unsqueeze(0))
#                 subbands['LH'].append(torch.tensor(LH, device=x.device).unsqueeze(0))
#                 subbands['HL'].append(torch.tensor(HL, device=x.device).unsqueeze(0))
#                 subbands['HH'].append(torch.tensor(HH, device=x.device).unsqueeze(0))
#
#         LL = torch.stack(subbands['LL'], dim=1).view(batch_size, channels, -1, width // 2)
#         LH = torch.stack(subbands['LH'], dim=1).view(batch_size, channels, -1, width // 2)
#         HL = torch.stack(subbands['HL'], dim=1).view(batch_size, channels, -1, width // 2)
