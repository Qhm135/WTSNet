...
parser.add_argument('--loadmodel', default='./ckpt/finetune_300.tar',
                    help='load model')
parser.add_argument('--error_vis', default='./error_vis/error_vis.png',
                    help='save error visualization')
parser.add_argument('--pred_disp', default='./pred_disp/pred_disp.png',
                    help='save pred_disp')
...
if args.loadmodel is not None:
    ...
    print(state_dict.keys())  # This will print the available keys: 'epoch', 'state_dict', 'train_loss', 'test_loss'

    # Loading the actual model state_dict
    model.load_state_dict(state_dict['state_dict'])
def main():
    for batch_idx, (img_L, img_R, dwt_img, disp_L) in enumerate(TestImgLoader):
        if batch_idx == 10:
            model.eval()
